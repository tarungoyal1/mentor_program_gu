...
require_once ('../includes/messagetocr.php');
...
if(Mentor::validate_mentor($mentor_id)){
        ...
        // uncomment following 2 lines of code in production
        // $mentor = Mentor::find_mentor_by_id($mentor_id); 
        // $recipient =  'pallavi.goel@galgotiasuniversity.edu.in';
       ...
       $recipient = "taruntube9@gmail.com";
        ...
        $bodyContent = '<h5>This e-mail is from a mentor at Galgotias University</h5>';
        $bodyContent .= '<p>From mentor: '.$mentor->employee_name.'</p>';
        ...
        if(!$mail->send()) {
            try {
               MessageToCR::insert_msg_mentor_cr($mentor_id, $email_sub, $email_body, "failed", $dtstamp, $ip);
            } catch (Exception $e) {}
            die("error||Error: Message could not be sent !");
        } else {

            try {
               MessageToCR::insert_msg_mentor_cr($mentor_id, $email_sub, $email_body, "sent", $dtstamp, $ip);
            } catch (Exception $e) {}
