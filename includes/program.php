...
class Program {
	...
	public static function find_all_programs() {
	global $db;
	...
	public static function get_course_name_by_id($cid=1) {
	global $db;
	$sanitized_cid = $db->string_prep($cid);
	$query = "SELECT course_name FROM programs WHERE id={$sanitized_cid} ";
	$result = $db->perform_query($query);
	$array = $db->fetch_array($result);
	return $array['course_name'];
	}
