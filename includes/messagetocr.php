...
class MessageToCR{
	...
	global $db;
	$query = "SELECT * FROM message_tocr ORDER BY msgTime DESC";
	...
	public static function find_msgs_by_guid($mid="") {
		...
		$result_array = self::find_by_sql("SELECT * FROM message_tocr WHERE from_guid='{$sanitized_mid}'");
    ...
    public function insert_msg_mentor_cr($mentor_id, $email_sub, $email_body, $status, $dtstamp, $ip) {
		global $db;
		...
		$query  = "INSERT INTO message_tocr (from_guid, msgSub, msgBody, msgTime, status, senderip) ";
